    ...
    <div className="flex flex-col gap-4">
      ...
      {sortedByEfficiency.length >= 2 && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <ChartCard title={`🏆 ${t("reports.topPerformers")}`} height={Math.max(200, topPerformers.length * 50)}>
            <div className="flex h-full flex-col gap-2">
              {topPerformers.map((e, i) => (
                <div key={e.worker_id} className="flex items-center gap-3 rounded-lg bg-emerald-50 px-3 py-2">
                  <span className={`flex h-7 w-7 items-center justify-center rounded-full font-bold text-white ${
                    i === 0 ? "bg-amber-500" : i === 1 ? "bg-slate-400" : i === 2 ? "bg-orange-500" : "bg-emerald-500"
                  }`}>{i + 1}</span>
                  <span className="flex-1 truncate text-sm font-semibold text-slate-700">{e.worker_name}</span>
                  <span className="text-sm font-bold text-emerald-600" dir="ltr">{e.efficiencyPercent.toFixed(0)}%</span>
                </div>
              ))}
            </div>
          </ChartCard>

          <ChartCard title={`🔻 ${t("reports.bottomPerformers")}`} height={Math.max(200, bottomPerformers.length * 50)}>
            <div className="flex h-full flex-col gap-2">
              {bottomPerformers.map((e) => (
                <div key={e.worker_id} className="flex items-center gap-3 rounded-lg bg-amber-50 px-3 py-2">
                  <span className="flex h-7 w-7 items-center justify-center rounded-full bg-amber-500 text-xs font-bold text-white">!</span>
                  <span className="flex-1 truncate text-sm font-semibold text-slate-700">{e.worker_name}</span>
                  <span className="text-sm font-bold text-amber-600" dir="ltr">{e.efficiencyPercent.toFixed(0)}%</span>
                </div>
              ))}
            </div>
          </ChartCard>
