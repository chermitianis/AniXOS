          ...
          {sortedRows.filter((r) => r.production_seconds > 0).length === 0 ? (
            ...
            <div className="flex h-full flex-col justify-center gap-2 overflow-y-auto">
              {sortedRows.filter((r) => r.production_seconds > 0).slice(0, 6).map((r) => {
                ...
                return (
                  <div key={r.machine_id} className="flex items-center gap-2 sm:gap-3">
                    <span className="w-20 shrink-0 truncate text-xs font-semibold text-slate-700 sm:w-32">
                      {r.machine_name}
                    </span>
                    ...
                    <span className="shrink-0 text-xs font-bold text-slate-600" dir="ltr">
                      {formatDuration(r.production_seconds)}
                    </span>
                  </div>
                );
              })}
        ...
        {sortedRows.length === 0 ? (
          ...
          <>
            ...
            <div className="flex flex-col gap-2 md:hidden">
              {sortedRows.map((r) => (
                <div key={r.machine_id} className="rounded-lg border border-slate-100 bg-slate-50/60 p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-bold text-slate-700">{r.machine_name}</div>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                    <div>
                      <div className="text-[10px] uppercase text-slate-400">
                        {t("setup.runningTimeShort")}
                      </div>
                      <div className="font-semibold text-blue-600" dir="ltr">
                        {formatDuration(r.production_seconds ?? 0)}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase text-slate-400">
                        {t("setup.downtimeLabel")}
                      </div>
                      <div className="font-semibold text-orange-600" dir="ltr">
                        {formatDuration(r.downtime_seconds ?? 0)}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase text-slate-400">
                        {t("setup.sessionsCount")}
                      </div>
