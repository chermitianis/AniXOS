    ...
    <div>
      <div className="mb-5 flex gap-2 rounded-xl border border-slate-200 bg-white p-1.5">
        <button
          ...
          className={`flex-1 rounded-lg px-2 py-2 text-xs font-bold sm:px-4 sm:text-sm ${activeTab === "task_types" ? "bg-blue-600 text-white" : "text-slate-500 hover:bg-slate-50"}`}
        >
        ...
        <button
          ...
          className={`flex-1 rounded-lg px-2 py-2 text-xs font-bold sm:px-4 sm:text-sm ${activeTab === "stop_reasons" ? "bg-orange-500 text-white" : "text-slate-500 hover:bg-slate-50"}`}
        >
      ...
      {activeTab === "task_types" ? (
        <div className="grid gap-4 lg:grid-cols-2 lg:gap-6">
          <form onSubmit={handleAddTaskType} className="h-fit rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
            <h2 className="mb-4 text-base font-bold text-slate-800 sm:text-lg">{t("setup.taskTypesList")} ({taskTypes.length})</h2>
            <ul className="flex flex-col gap-2">
              {taskTypes.map((tt) =>
                editingTtId === tt.id ? (
                  <li key={tt.id} className="rounded-lg border border-slate-200 bg-slate-50 p-2.5">
                    <input value={editTtName} onChange={(e) => setEditTtName(e.target.value)} className="mb-2 w-full rounded border border-slate-300 px-2 py-1.5 text-sm" />
                  ...
                  <li key={tt.id} className="rounded-lg px-3 py-2 text-sm text-white" style={{ backgroundColor: tt.color }}>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex shrink-0 items-center gap-1.5">
                        ...
                        <button onClick={() => startEditTaskType(tt)} className="rounded-lg bg-white/20 p-1.5"><Pencil size={12} /></button>
                        <button onClick={() => deleteTaskType(tt.id)} className="rounded-lg bg-white/20 p-1.5"><Trash2 size={12} /></button>
                      </div>
                    </div>
                  </li>
                )
        ...
        <div className="grid gap-4 lg:grid-cols-2 lg:gap-6">
          <form onSubmit={handleAddStopReason} className="h-fit rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
            <h2 className="mb-4 text-base font-bold text-slate-800 sm:text-lg">{t("setup.stopReasonsList")} ({stopReasons.length})</h2>
            <ul className="flex flex-col gap-2">
              {stopReasons.map((r) =>
                editingSrId === r.id ? (
                  <li key={r.id} className="rounded-lg border border-slate-200 bg-slate-50 p-2.5">
                    <input value={editSrName} onChange={(e) => setEditSrName(e.target.value)} className="mb-2 w-full rounded border border-slate-300 px-2 py-1.5 text-sm" />
                    <div className="mb-2 flex flex-wrap gap-1.5">
                      {STOP_COLORS.map((c) => (
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => saveEditStopReason(r.id)} className="flex-1 rounded-lg bg-green-600 py-1.5 text-xs font-bold text-white"><Check size={14} className="mx-auto" /></button>
                      <button onClick={() => setEditingSrId(null)} className="flex-1 rounded-lg bg-slate-300 py-1.5 text-xs font-bold text-white"><X size={14} className="mx-auto" /></button>
                    </div>
                  </li>
                ) : (
                  <li key={r.id} className="rounded-lg px-3 py-2 text-sm text-white" style={{ backgroundColor: r.color }}>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex shrink-0 items-center gap-1.5">
                        ...
                        <button onClick={() => startEditStopReason(r)} className="rounded-lg bg-white/20 p-1.5"><Pencil size={12} /></button>
                        <button onClick={() => deleteStopReason(r.id)} className="rounded-lg bg-white/20 p-1.5"><Trash2 size={12} /></button>
                      </div>
                    </div>
                  </li>
                )
